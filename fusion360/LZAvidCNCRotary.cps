/**
	Copyright © 2019 by Latency: Zero, LLC.
	All rights reserved.

	Latency: Zero, LLC Avid CNC router post processor configuration (linuxcnc).
	
	Based on Autodesk’s linuxcnc.cps
*/

description = "LZ Avid CNC Rotary";
vendor = "LZ";
vendorUrl = "https://latencyzero.com/";
legal = "Copyright © 2019 by Latency: Zero, LLC.";
certificationLevel = 2;
minimumRevision = 40783;

longDescription = "Latency: Zero, LLC Avid CNC router post.";
...
/**
	This is where we handle Manual NC/Action
*/

function
onParameter(inName, inValue)
{
	if (inName == "action" && inValue == "moveHomeAndPause")
	{
		onCommand(COMMAND_STOP_SPINDLE)
		writeBlock(gFormat.format(53), gMotionModal.format(0), zOutput.format(-10.0))
		writeBlock(gFormat.format(53), gMotionModal.format(0), xOutput.format(10.0), yOutput.format(-10.0))
...
function setWorkPlane(abc) {
	...
	if (!retracted) {
		writeComment("setWorkPlane retract Z");
		writeRetract(Z);
	}
...
function onSection() {
	...
	if (insertToolCall || newWorkOffset || newWorkPlane) {
		...
		writeComment("Section retract Z");
		writeRetract(Z);
	}
...
{
	setCoolant(COOLANT_OFF);
	writeComment("retract Z:");
	writeRetract(Z);

	writeComment("set work plane:");
	setWorkPlane(new Vector(0, 0, 0)); // reset working plane

	writeComment("retract X, Y:");
	writeRetract(X, Y);
